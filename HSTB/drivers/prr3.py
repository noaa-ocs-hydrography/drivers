...
class X7kRead:
    ...
    def __init__(self, infilename, autoplot=True):
        ...
        self.loc = 0  # the current file byte location
    ...
    def findval(self):
        ...
        oldval = self.packet.ping7000.header[13]
        self.findpacket(7000)
        val = self.packet.ping7000.header[13]
        while val == oldval:
            self.findpacket(7000)
            oldval = val
            val = self.packet.ping7000.header[13]
    ...
    def extract(self):
        ...
        outfilename = self.inname + '_prr.s7k'
        ...
        while count < 6:
            ...
            if self.packet.datatype == 7000:
                count += 1
        end = self.infile.tell()
        self.infile.seek(0)
        outfile.write(self.infile.read(end))
        self.infile.seek(0)
        outfile.close()
        ...
        while ptr < flen:
            ...
            if ptr + outfilesize < flen:
                ...
                if ftype == '7k':
                    ...
                    raise NotImplementedError(f'expected 7k or s7k, got {ftype}')
...
class DataFrame:
    ...
    def gettime(self):
        ...
        temptime = str(self.header[6:8])[1:-1] + ', ' + str(self.header[9:11])[1:-1]
        timestruct = time.strptime(temptime, self.time_fmt)
        utctime = calendar.timegm(timestruct) + self.header[8]
        return utctime
...
class BaseData(object):
    ...
    def get_display_string(self):
        result = ""
        for i in range(min(len(self.label), len(self.header))):
